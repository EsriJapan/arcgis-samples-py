...
def run_select_tool(input,field,outputws,prefix,val):
    ...
    # 対象フィールドが文字列以外
    if target_field.type != u"String":
        outname = prefix + str(val)
        # 条件式の生成
        exp = u'"{0}" = {1}'.format(field,val)
        # 出力がフォルダ
        if out_desc.workspaceType == u"FileSystem":
          output = outputws + os.sep + outname + u".shp"
        # 出力がファイルジオデータベース
        elif out_desc.workspaceType == u"LocalDatabase":
          output = outputws + os.sep + outname
    # 対象フィールドが文字列
    elif target_field.type == u"String":
        # 数字から始まる場合
        if val.encode("utf-8")[0].isdigit():
            outname = prefix + str(val)
            # 条件式の生成
            exp = u'"{0}" = {1}'.format(field,val)
            # 出力がフォルダ
        ...
        # それ以外
            ...
            # 出力がフォルダ
            if out_desc.workspaceType == u"FileSystem":
              output = outputws + os.sep + prefix + u".shp"
            # 出力がファイルジオデータベース
            elif out_desc.workspaceType == u"LocalDatabase":
...
def main():
    ...
    for val in dic:
        ...
        # 処理状況をプログレスウィンドウに表示
        arcpy.SetProgressor("default",u"{0} ({1} / {2})".format(val,i+1,len(dic)))
        i += 1

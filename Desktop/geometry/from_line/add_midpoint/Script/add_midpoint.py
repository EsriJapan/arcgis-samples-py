...
def main():
    ...
    single_output = arcpy.GetParameter(2)
    ...
    with  arcpy.da.SearchCursor(input_line,search_fields_name) as input_cur:
        ...
        for row in input_cur:
            ...
            # GDB は NULL 値を格納可能
                ...
                newValue = row[:-1]
            """
            Polyline::postionAlongLine メソッドで指定した距離や
            パーセンテージ上の位置をポイントとして取得可能
            """
            mid_point = geom.positionAlongLine (0.5,True)
            new_row = tuple(newValue) + (mid_point,)
